...
class Chain(serial.Serial):
    ...
    NE-1000 syringe pumps are daisy chained together in a 'pump chain'
    ...
    def __init__(self, port):
        serial.Serial.__init__(self,port=port, stopbits=serial.STOPBITS_ONE, parity=serial.PARITY_NONE, timeout=1)
        self.flushOutput()
        self.flushInput()
        logging.info('Chain created on %s',port)

class NE1000:
    """Create Pump object for NE-1000.
    ...
    Optional arguments:
        ...
        name: used in logging. Default is NE-1000.
    """
    def __init__(self, chain, address=0, name='NE-1000'):
        self.name = name
        ...
        try:
            command = f"{address} VER\r".encode('ascii')
            self.write(command)
            resp = self.read(100).decode('ascii', errors='ignore').strip()
    ...
    def write(self,command):
        self.serialcon.write(f"{self.address} {command}\r".encode('ascii'))
    ...
    def setdiameter(self, diameter):
        ...
        NE-1000 syringe diameter range is 0.10 to 50.00 mm. Maximum of 4 digits 
        plus 1 decimal point. Maximum of 3 digits to the right of the decimal point.
        """
        ...
        if len(diameter) > 5:
            diameter = diameter[0:4] # 4 digits and 1 decimal point
    ...
    def setflowrate(self, flowrate):
        ...
        Flow rate is converted to a string. NE-1000 requires it to have
        ...
        range. This depends on the syringe diameter. See NE-1000 manual.
        """
...
class PHD2000(Pump):
    ...
    stick to the NE-1000 protocol with commands to stop and set the
    target volume.
    """
    def stop(self):
        """Stop pump."""
